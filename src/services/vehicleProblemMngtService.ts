...
export const vehicleProblemMngtService = {
  ...
  getVehicleProblems: async (
    query: VehicleProblemQuery = {}
  ): Promise<VehicleProblem[]> => {
    ...
    return problems.map(
      (problem: VehicleProblemResponse): VehicleProblem => ({
        id: problem.id?.toString() || "",
        vehicleId:
          problem.vehicle_id?.toString() || problem.vehicle?.toString() || "",
        title: problem.title || "",
  ...
  getVehicleProblemById: async (problemId: string): Promise<VehicleProblem> => {
    const response = await apiGet<VehicleProblemResponse>(
      `/shop/vehicle-problems/${problemId}/`
    );

    return {
      id: response.id?.toString() || "",
      vehicleId:
        response.vehicle_id?.toString() || response.vehicle?.toString() || "",
      ...
      vehicle: response.vehicle_details
        ? {
            ...
            isActive: response.vehicle_details.is_active ?? true,
          }
        : undefined,
    };
  },
  ...
  createVehicleProblem: async (
    problemData: CreateVehicleProblemData
  ): Promise<VehicleProblem> => {
    const createData = {
    ...
    const response = await apiPost<VehicleProblemResponse>(
      "/shop/vehicle-problems/",
      createData
    );
  ...
  updateVehicleProblem: async (
    problemId: string,
    problemData: UpdateVehicleProblemData
  ): Promise<VehicleProblem> => {
    const updateData: Partial<VehicleProblemResponse> = {};

    if (problemData.vehicleId !== undefined)
      updateData.vehicle_id = problemData.vehicleId;
    if (problemData.title !== undefined) updateData.title = problemData.title;
    if (problemData.description !== undefined)
      updateData.description = problemData.description;
    if (problemData.severity !== undefined)
      updateData.severity = problemData.severity;
    if (problemData.status !== undefined)
      updateData.status = problemData.status;
    if (problemData.estimatedCost !== undefined)
      updateData.estimated_cost = problemData.estimatedCost;
    if (problemData.actualCost !== undefined)
      updateData.actual_cost = problemData.actualCost;
    if (problemData.notes !== undefined) updateData.notes = problemData.notes;
    if (problemData.resolvedDate !== undefined)
      updateData.resolved_date = problemData.resolvedDate;

    const response = await apiPut<VehicleProblemResponse>(
      `/shop/vehicle-problems/${problemId}/`,
      updateData
    );
  ...
  getUnresolvedProblems: async (): Promise<VehicleProblem[]> => {
    const response = await apiGet<VehicleProblemListResponse>(
      "/shop/vehicle-problems/unresolved/"
    );
    const problems = response.results || response;
  ...
  resolveProblem: async (
    ...
    return vehicleProblemMngtService.updateVehicleProblem(
      problemId,
      updateData
    );
  },
